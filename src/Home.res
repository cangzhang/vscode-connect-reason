...
let make = () => {
  ...
  let queries = ["response_type=code", "client_id=" ++ clientId, "scope=" ++ scope, "state=" ++ state, "callbackUri=" ++ callbackUri];
  ...
  <main>
    ...
    {
      hasTeam ? <a href={authUrl.contents ++ authQuery } target="_blank">{React.string("Authorize")}</a> 
